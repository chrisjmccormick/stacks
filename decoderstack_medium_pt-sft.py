...
class Hyperparameters:
    # data
    data_path = os.environ.get("DATA_PATH", ".")
    train_files: str = os.path.join(data_path, f"data/{DATASET_NAME}/{PT_DATA_SUBDIR}/train_*.bin")
    val_files: str = os.path.join(data_path, f"data/{DATASET_NAME}/{PT_DATA_SUBDIR}/val_*.bin")
...
# Batch size and scaling matching nanochat d24 standard config
total_batch_size = 2**20  # 1,048,576 tokens (nanochat auto-computes this via Power Lines scaling for d24)
grad_accum_steps = total_batch_size // (16 * args.train_max_seq_len * world_size)  # = 4 (16 seqs/GPU, matching nanochat d24)
grad_scale = 1 / grad_accum_steps
batch_lr_scale = (total_batch_size / 2**19) ** 0.5  # √(B/B_ref) = √2 ≈ 1.4142

# Scale weight decay based on model size.
weight_decay_scaled = 0.06  # nanochat scales 0.28 → 0.059738 for d24 via T_epoch framework

print0(f"Total batch size: {total_batch_size:,} tokens", console=True)
print0(f"Grad accum steps: {grad_accum_steps}", console=True)
print0(f"Grad scale: {grad_scale}", console=True)
print0(f"Batch LR scale: {batch_lr_scale}", console=True)
print0(f"Weight decay scaled: {weight_decay_scaled}", console=True)

# --- Initialize the Optimizer ---
optimizer = model.setup_optimizer(
    unembedding_lr=0.008 * batch_lr_scale,
