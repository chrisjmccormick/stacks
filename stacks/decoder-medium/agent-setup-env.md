...
```bash
#!/bin/bash
# Fill in your actual tokens/keys below, then on each fresh instance run:
#   source env.sh
...
cd ~/
git clone https://${GITHUB_TOKEN}@github.com/chrisjmccormick/stacks.git
git -C stacks remote set-url origin https://${GITHUB_TOKEN}@github.com/chrisjmccormick/stacks.git
```

Everything below runs from **this** project folder, not the repo root:

```bash
cd ~/stacks/stacks/decoder-medium
```
