# probe_embed.py
#
# Trying: train_stack_log_v1.py (stacks@e1e1459, bigram-embeds) with the training loop
# replaced by a probe of the step-1 loss jump (6.05 -> 7.65): the § Probe section at the
# end. Everything above it is the instrumented script unchanged, except wandb and
# checkpoints are off. Assembled by make_probe.py.
#
...
class StackConfig:
    ...
    run_name:        str = "probe_embed"  # both wandb and log files
...
# ==============================================================================
# § Probe: how does step 0's input_embeds update raise step 1's loss?
# ==============================================================================
# Replaces § Training Loop. probe_step1.py found that step 0's update lowers the loss
# on step 1's batch when input_embeds is held out (6.077 -> 6.017) and raises it to
# 7.645 when it is applied. This probe applies the real step 0 to everything, then
# varies only what happens to input_embeds: which rows take the update (by how often
# the token occurred in step 0's batch), the update's shape (Adam's sign step vs the
# gradient direction at the same per-row size), and its size. Then it runs the real
# step 1 and checks the branch's step-2 loss against bigram_lambdas' sign flip.
...
batches = [next(train_loader) for _ in range(3 * cfg.grad_accum_steps)]   # steps 0, 1 and 2
train_loader.close()
torch.cuda.synchronize()

MUON  = ("W_Q", "W_K", "W_V", "W_O", "W_in", "W_out", "attn_gate", "ve_gate")
ADAMW = ("lm_head", "input_embeds", "value_embeds", "bigram_embeds", "resid_lambdas",
         "x0_lambdas", "bigram_lambdas", "smear_gate", "smear_lambda", "backout_lambda")
ALL = MUON + ADAMW

snap = {p.name: {attr: getattr(p, attr).cpu() for attr in ("w", "mantissa", "first_mntm", "scnd_mntm")
                 if getattr(p, attr) is not None} for p in m}

def eval_losses(which=(0, 1, 2, 3)):
    out = []
    for i in which:
        inputs, targets, cu_seqlens = batches[i]
        nats = forward_backward(inputs, targets, cu_seqlens, micro_step=0, backward=False)
        out.append(nats.float().mean().item())
    return out

def real_step(step):
    """Forward/backward over the step's two micro-batches; the grads are left in place."""
...
print(f"step 0 train loss (micro 1): {real_step(0):.6f}   (bi_embed_log_v1 logged 6.048965)", flush=True)
grads = {p.name: p.grad.cpu() for p in m}

def restore():
    for p in m:
        for attr, saved in snap[p.name].items():
            getattr(p, attr).copy_(saved)
        p.grad.copy_(grads[p.name])

def apply_step0(names, lr_scale={}):
    for p in m:
        if p.name not in names:
            continue
        saved_lr = p.lr_bc_t.clone()
        p.lr_bc_t.mul_(lr_scale.get(p.name, 1.0))
        (muon_step_fused if p.name in MUON else adamw_step_fused)(p, p.grad, t_step)
        p.lr_bc_t.copy_(saved_lr)

# --- Token frequency in step 0's batch, and how each row's direction moved ---
V = cfg.d_vocab
counts0 = torch.bincount(torch.cat([batches[0][0], batches[1][0]]).long(), minlength=V)
counts1 = torch.bincount(torch.cat([batches[2][0], batches[3][0]]).long(), minlength=V)
embed0 = snap["input_embeds"]["w"].to(device).float()
restore(); apply_step0(ALL)
embed1 = m.input_embeds.w.float()
cos = F.cosine_similarity(embed0, embed1, dim=1)
angle = torch.rad2deg(torch.acos(cos.clamp(-1, 1)))
step_norm = (embed1 - embed0).norm(dim=1)
buckets = [(0, 1, "0 (absent)"), (1, 2, "1"), (2, 4, "2-3"), (4, 16, "4-15"), (16, 256, "16-255"),
           (256, 4096, "256-4095"), (4096, 10**9, "4096+")]
T1 = counts1.sum().item()
print("\nRows of input_embeds by the token's count in step 0's batch (2 x 262,144 tokens):")
print("| count in step 0 | rows | share of step 1's tokens | mean row rotation | mean step L2 (row L2 = 22.2) |")
print("|---|---|---|---|---|")
for lo, hi, label in buckets:
    sel = (counts0 >= lo) & (counts0 < hi)
    n = sel.sum().item()
    if n == 0:
        continue
    print(f"| {label} | {n} | {100 * counts1[sel].sum().item() / T1:.1f}% | {angle[sel].mean().item():.1f} deg "
          f"| {step_norm[sel].mean().item():.2f} |")
print(flush=True)
del embed1

# --- Variants: the real step 0 everywhere, with input_embeds' update varied ---
grad_embed = grads["input_embeds"].to(device).float()             # flipped sign: the update's direction
row_gnorm = grad_embed.norm(dim=1, keepdim=True)
sign_norm = 0.3 * cfg.d_model ** 0.5                              # L2 of the +-0.3 sign step per row

def set_embed_rows(rows_mask):
    """Undo input_embeds' update on the rows where rows_mask is False."""
    m.input_embeds.w[~rows_mask] = embed0[~rows_mask].to(torch.bfloat16)

def grad_direction_step(scale):
    """input_embeds := init + scale * (the gradient direction, per row, at the sign step's L2)."""
    step = sign_norm * scale * grad_embed / row_gnorm.clamp_min(1e-30)
    m.input_embeds.w.copy_((embed0 + step).to(torch.bfloat16))

variants = []
variants.append(("all (the real step 0)", lambda: None))
variants.append(("all but input_embeds", lambda: set_embed_rows(torch.zeros(V, dtype=torch.bool, device=device))))
for k in (2, 4, 16, 64, 256, 1024):
    variants.append((f"all, input_embeds update only on rows with count >= {k}", lambda k=k: set_embed_rows(counts0 >= k)))
for k in (2, 4, 16, 64, 256, 1024):
    variants.append((f"all, input_embeds update only on rows with count < {k}", lambda k=k: set_embed_rows(counts0 < k)))
for s in (1.0, 0.5, 0.25, 0.1, 0.03):
    variants.append((f"all, input_embeds step = gradient direction at {s} x the sign step's row L2", lambda s=s: grad_direction_step(s)))

rows = []
print("| variant | micro 0 | micro 1 (= step 0 log) | micro 2 | micro 3 (= step 1 log) |")
print("|---|---|---|---|---|", flush=True)
for label, tweak in variants:
    restore(); apply_step0(ALL); tweak()
    losses = eval_losses()
    rows.append((label, *losses))
    print(f"| {label} | " + " | ".join(f"{l:.4f}" for l in losses) + " |", flush=True)
for s in (0.5, 0.3, 0.2, 0.1, 0.05, 0.02, 0.01):
    restore(); apply_step0(ALL, {"input_embeds": s})
    losses = eval_losses()
    label = f"all, input_embeds lr x{s} (sign step of {0.3 * s:.3g} per element)"
    rows.append((label, *losses))
    print(f"| {label} | " + " | ".join(f"{l:.4f}" for l in losses) + " |", flush=True)

with open(f"logs/{cfg.run_name}_variants.csv", "w", newline="") as f:
    wr = csv.writer(f)
    wr.writerow(["variant", "micro0", "micro1", "micro2", "micro3"])
    wr.writerows(rows)
print(f"rows -> logs/{cfg.run_name}_variants.csv", flush=True)

# --- The branch's step 2: bigram_lambdas' sign flip at step 1 ---
# In bi_embed_log_v1 the loss at step 2 was 7.264 against ~6.57 in every no-table run.
# bigram_lambdas took their first (negative) gradient at step 1 and Adam's step carried
# them from 0.10 to -0.256. Run the real steps 0 and 1, then score step 2's batch with
# bigram_lambdas as trained, held at their init, and zeroed.
restore(); apply_step0(ALL)
for p in m:
    p.grad.zero_()
t_step.add_(1)
print(f"\nstep 1 train loss (micro 3): {real_step(1):.6f}   (bi_embed_log_v1 logged 7.645220)")
for p in m:
    (muon_step_fused if p.name in MUON else adamw_step_fused)(p, p.grad, t_step)
    p.grad.zero_()
print(f"bigram_lambdas after step 1: {[round(v, 4) for v in m.bigram_lambdas.w.tolist()]}   (traced L00/L05/L10: -0.2562)")
after_step1 = m.bigram_lambdas.w.clone()
print("\n| step 2's batch, after the real steps 0 and 1 | micro 4 | micro 5 (= step 2 log) |")
print("|---|---|---|")
for label, value in (("bigram_lambdas as trained", after_step1), ("bigram_lambdas held at 0.1", torch.full_like(after_step1, 0.1)),
                     ("bigram_lambdas = 0 (table off)", torch.zeros_like(after_step1))):
    m.bigram_lambdas.w.copy_(value)
    l4, l5 = eval_losses((4, 5))
    print(f"| {label} | {l4:.4f} | {l5:.4f} |", flush=True)
