# train_stack-02-lazy-table.py -- stacks@bigram-embeds d75f72d train_stack.py, plus:
# Trying: the bigram table stepped on the rows the batch touched only (adamw_step_rows,
# § Optimizer Math), with a per-row bias correction, so a slot's element no longer drifts
# for ~10 steps after each hit on Adam's replayed momentum, and a row's first hit late
# in the run is the same +-lr step as at step 0. Written by make_lazy_table.py. The traced
# m_sqrt_v for bigram_embeds still uses the run's step count for its bias correction, so
# it reads wrong for this parameter; param and grad are exact.
...
class StackConfig:
    ...
    run_name:        str = "bi_embed_02_lazy_table"  # both wandb and log files
    use_wandb:       bool = True
...
def adamw_step_rows(
    p: Param,
    grad: Tensor,       # (rows, D), zero on every row the batch did not touch
    t: Tensor,          # (1,) Current step for the lr and decay schedules
    n_updates: Tensor,  # (rows, 1) int32: how many updates each row has had, this one included
) -> None:
    """AdamW on the rows the batch touched only. A row with a zero gradient keeps its
    weights and moments, so the momentum of its last hit is not replayed as a drift over
    the following steps; and each row's bias correction counts its own updates rather
    than the run's steps, so a row's first hit at step 500 is the same +-lr sign step as
    one at step 0. p.lr_bc_t must then carry no global bias correction."""

    grad = grad.float()
    hit = (grad != 0).any(dim=1, keepdim=True)     # (rows, 1)
    n_updates.add_(hit.to(n_updates.dtype))

    # ==== Buffer Update, hit rows only ====
    p.first_mntm.copy_(torch.where(hit, p.first_mntm * p.mntm_b1_t[t] + grad * p.grad_b1_t[t], p.first_mntm))
    p.scnd_mntm.copy_(torch.where(hit, p.scnd_mntm * p.mntm_b2_t[t] + grad.square() * p.grad_b2_t[t], p.scnd_mntm))

    # ==== Parameter Update, hit rows only, with the row's own bias correction ====
    n = n_updates.clamp_min(1).float()
    bias = (1.0 - p.mntm_b2_t[t] ** n).sqrt() / (1.0 - p.mntm_b1_t[t] ** n)      # (rows, 1)
    master = p.w.float()
    update = p.lr_bc_t[t] * bias * (p.first_mntm / (p.scnd_mntm.sqrt() + p.eps_t[t]))
    p.w.copy_(torch.where(hit, master * p.wd_t[t] + update, master))
...
# The bigram table is stepped by adamw_step_rows (hit rows only, per-row bias
# correction), so its lr table carries the schedule and the peak lr alone.
m.bigram_embeds.lr_bc_t.copy_(dev(lr_mult_t * 0.9))
bigram_hits = torch.zeros(cfg.d_bigram, 1, dtype=torch.int32, device=device)
...
for step in range(cfg.num_steps + 1):
    ...
    # The bigram table: AdamW on the rows this step's batch touched only.
    adamw_step_rows(m.bigram_embeds, m.bigram_embeds.grad, t_step, bigram_hits)
