# probe_firstmove3.py
#
# Trying: whether lm_head's consistent early move (a rank-1 shrink of its unigram
# component, probe_firstmove2.py) can be built from the init head and corpus counts alone,
# and how those inits score on held-out batches (§ Probe at the end). Above it: the
# instrumented v2 script up to its weight init, wandb off. Assembled by
# make_firstmove_probe3.py.
#
...
# ==============================================================================
# § Probe: can the head's early move be built from the init head and corpus counts alone?
# ==============================================================================
# probe_firstmove2.py found lm_head's early move is c u^T with u fixed over steps 5-20 and
# c a line in log frequency (weighted R^2 0.91): the model shrinks the head's unigram
# component by about half, and giving it to the init cuts the step-1 jump and leads at
# step 5. u and c there came from the trajectory. Here: regress the init head's rows on log
# frequency to get the direction beta along which the init encodes frequency, compare it
# with u, and score inits that shrink that component by a fraction alpha, on held-out
# batches (steps 20-25), against the trajectory-derived version.
...
def rank1(delta):
    ...
    return Vh[0], Uh[:, 0] * s[0] / sw[:, 0], (s[0].square() / s.square().sum()).item()
...
batches = [next(train_loader) for _ in range(26 * cfg.grad_accum_steps)]
train_loader.close()
...
def train_steps(n, start=0, snap_at=()):
    ...
    for step in range(n):
        ...
        if step + 1 in snap_at:
            snaps[step + 1] = rebuild_master(m.lm_head.w, m.lm_head.mantissa).cpu()
    return losses, snaps

H0 = rebuild_master(m.lm_head.w, m.lm_head.mantissa).clone()

# --- The init head's own frequency direction: H0_v ~ hbar + (log f_v - mbar) beta ---
mbar = (w * log_freq).sum() / w.sum()
hbar = (w.unsqueeze(1) * H0).sum(0) / w.sum()
lf = (log_freq - mbar).unsqueeze(1)
beta = ((w.unsqueeze(1) * lf * (H0 - hbar)).sum(0) / (w * lf.squeeze(1).square()).sum())      # (D,)
beta_hat = beta / beta.norm()
freq_part = lf * beta                                                                          # (V, D): the frequency component
resid = H0 - hbar - freq_part
print(f"|beta| {beta.norm().item():.4f}; token-weighted share of the init head's centred energy that is its frequency component: "
      f"{((w.unsqueeze(1) * freq_part.square()).sum() / (w.unsqueeze(1) * (H0 - hbar).square()).sum()).item():.3f}")
print(f"cos(beta, mean init head row) {F.cosine_similarity(beta, hbar, dim=0).item():+.3f}")

# --- The trajectory's version, for comparison ---
losses, snaps = train_steps(20, snap_at=(10,))
print("logged loss, steps 0-19: " + " ".join(f"{l:.3f}" for l in losses))
H10 = snaps[10].to(device)
u, c, share = rank1(H10 - H0)
if F.cosine_similarity(u, beta_hat, dim=0) < 0:
    u, c = -u, -c
print(f"\ncos(u, beta): {F.cosine_similarity(u, beta_hat, dim=0).item():+.3f}   (u: the rank-1 direction of H10 - H0, share {share:.2f})")
print(f"| | {bucket_header} |")
print("|---|" + "---|" * (len(buckets) + 1))
print(f"| H0 . beta_hat: the init head's component along beta | {by_bucket(H0 @ beta_hat)} |")
print(f"| (H10 - H0) . beta_hat: the move along beta | {by_bucket((H10 - H0) @ beta_hat)} |")
print(f"| c: the move along u | {by_bucket(c)} |")
print(f"| ratio: move along beta / init component along beta (beyond the mean) | {by_bucket(((H10 - H0) @ beta_hat) / (freq_part @ beta_hat).clamp(-1e9, 1e9).where((freq_part @ beta_hat).abs() > 0.05, torch.full_like(c, float('nan'))), fmt='{:.2f}')} |", flush=True)

# --- Candidate inits on held-out batches ---
print("\n## candidate inits on held-out batches (steps 20-25; step 0 = the init on that batch)")
print("| init | step 0 | 1 | 2 | 3 | 4 | 5 |")
print("|---|---|---|---|---|---|---|", flush=True)
def candidate(label, H):
    restore()
...
candidate("the init as is", H0)
candidate("H0 + c u^T (from the trajectory; probe_firstmove2's best head-only init)", H0 + c.unsqueeze(1) * u)
for alpha in (0.25, 0.5, 0.75, 1.0):
    candidate(f"H0 - {alpha} x its frequency component (log f - mean) beta", H0 - alpha * freq_part)
for shift in (0.3, 0.6):
    candidate(f"H0 - 0.5 x its frequency component, all rows shifted +{shift} along beta_hat", H0 - 0.5 * freq_part + shift * beta_hat)
    candidate(f"H0 - 0.5 x its frequency component, all rows shifted -{shift} along beta_hat", H0 - 0.5 * freq_part - shift * beta_hat)
