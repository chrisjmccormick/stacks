# train_stack_log_v2-03-freeze-embed-head.py
#
# Trying: input_embeds and lm_head frozen for steps 0-9 (no update, grads zeroed), then trained
# on their own optimizer clock. Does letting the rest of the model move first change how
# the embeddings' opening sign step lands, and what does the loss do at steps 10-11?
# Aborts at step 50 (validation at 0 and 50), no checkpoints. Everything below this
...
class StackConfig:
    ...
    run_name:        str = "log_v2_03_freeze_embed_head10"  # both wandb and log files
    use_wandb:       bool = True
...
# The frozen parameters' own clock: starts when they thaw at step 10, so their first
# update gets a first step's bias correction, as at step 0 of a normal run.
t_step_thawed = torch.zeros(1, dtype=torch.int64, device=device)
...
for step in range(cfg.num_steps + 1):
    ...
    for p in (m.lm_head, m.input_embeds, m.value_embeds, m.resid_lambdas, m.x0_lambdas, \
        ...
        # Frozen for the first 10 steps, then trained on their own clock.
        if p.name in ("lm_head", "input_embeds",):
            if step < 10:
                p.grad.zero_()
                continue
            adamw_step_fused(p, p.grad, t_step_thawed)
        else:
