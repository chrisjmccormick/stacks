# train_stack_log_v2-01-embed-warmup.py
#
# Trying: train_stack_log_v2.py with input_embeds' learning rate warmed up linearly over
# the first 20 steps (0.05 -> 1.0 of peak). Adam's first step is a full +-peak_lr sign
# step on every element it touches; at peak lr 0.3 that rotates ~80% of the prior-
# initialised embedding rows by ~20 degrees and lifts the loss from 6.05 at step 0 to
# 7.6 at step 1 (probe_step1.py in the step1-loss-jump session). Held at a tenth of
# the lr, that step is harmless. Everything below this paragraph is v2's description.
#
...
class StackConfig:
    ...
    run_name:        str = "log_v2_01_embed_warmup20"  # both wandb and log files
    use_wandb:       bool = True
...
# input_embeds only: linear warmup over the first 20 steps, so Adam's opening sign
# steps land at 0.015-0.3 per element instead of 0.3 from step 0.
embed_warmup_t = np.minimum(1.0, steps_1idx / 20.0)
...
for (name, w, peak_lr, b1_grad, b2_grad, wd, slots) in embed_configs:
    ...
    warmup_t = embed_warmup_t if name == "input_embeds" else 1.0
    ...
    p = Param(
        ...
        # Schedules
        # Fold bias correction into the learning rate.
        lr_bc_t      = dev(warmup_t * lr_mult_t * peak_lr * (1.0 - b2_mntm ** steps_1idx) ** 0.5 / (1.0 - b1_mntm ** steps_1idx)),

        # Weight decay schedule
        wd_t         = dev(1.0 - warmup_t * lr_mult_t * peak_lr * wd),
