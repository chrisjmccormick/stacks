# train_stack_log_v2-05-embed-beta-warmup.py
#
# Trying: input_embeds' (1-beta) grad mix-ins warmed up over steps 1-10. Step 0 keeps the
# full mix-ins, so the moments are initialised from a real gradient (the step-0 update
# is unchanged); steps 1-10 ramp both mix-ins linearly from 0.01 x (1-beta) to (1-beta),
# so the large restoring gradients of steps 1-2 enter m and v at a fraction of their
# weight and v keeps step 0's magnitude instead of inflating and having to recover.
# The betas (the decays) and the lr are untouched. Everything below this paragraph is
# v2's description.
#
...
class StackConfig:
    ...
    run_name:        str = "log_v2_05_embed_beta_warmup10"  # both wandb and log files
    use_wandb:       bool = True
...
# input_embeds only: multiplier on the (1-beta) grad mix-ins. Full at step 0 to initialise
# the moments, then a 10-step linear ramp from 0.01 back to 1 over steps 1-10.
embed_grad_mult_t = np.ones(cfg.num_steps)
embed_grad_mult_t[1:11] = np.linspace(0.01, 1.0, 10)
...
for (name, w, peak_lr, b1_grad, b2_grad, wd, slots) in embed_configs:
    ...
    grad_mult_t = embed_grad_mult_t if name == "input_embeds" else np.ones(cfg.num_steps)
    ...
    p = Param(
        ...
        # Beta schedule: decays constant, the mix-ins ramped for input_embeds
        mntm_b1_t    = dev(np.full(cfg.num_steps, b1_mntm)),   # first_mntm decay (Beta1)
        grad_b1_t    = dev(grad_mult_t * b1_grad)             ,   # first_mntm grad mix-in (1-Beta1)

        mntm_b2_t    = dev(np.full(cfg.num_steps, b2_mntm)),   # scnd_mntm decay (Beta2)
        grad_b2_t    = dev(grad_mult_t * b2_grad)             ,   # scnd_mntm grad mix-in (1-Beta2)
...
#             (the direction the update pushes the weight). resid/x0/backout
#             lambdas only collect it on the last micro-batch.
