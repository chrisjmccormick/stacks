# train_stack_log_v2-06-head-freq-half.py
#
# Trying: lm_head's log-frequency component halved at init. The baseline's first ten steps
# do this on their own: the head's early move is rank-1 along one direction, with a
# per-token coefficient that is a line in log frequency, and it halves the component the
# closed-form init put there. Done at init it removes the step-1 loss jump (the init is
# 0.75 nats worse, the next step 1.25 nats better) and leads by 0.15 nats at step 5 on
# held-out batches (step1-loss-jump session, probe_firstmove3.py). Everything below this
# paragraph is v2's description.
#
...
class StackConfig:
    ...
    run_name:        str = "log_v2_06_head_freq_half"  # both wandb and log files
    use_wandb:       bool = True
...
# Halve the head's log-frequency component: regress the rows on log frequency (weighted by
# frequency, as the head itself was fitted) and subtract half of the fitted part.
token_counts = torch.tensor(np.load(os.path.join(DATASET_DIR, "tokenizer/token_counts.npz"))["train"],
                            dtype=torch.float32, device=device)
per_step = token_counts / token_counts.sum() * cfg.total_batch_size          # occurrences per training step
freq_weight = per_step.clamp_min(1e-3)
...
del token_counts, per_step, freq_weight, log_freq, head_mean, freq_direction
