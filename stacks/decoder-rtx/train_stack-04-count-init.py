# train_stack-04-count-init.py -- stacks@bigram-embeds d75f72d train_stack.py, plus:
# Trying: the bigram table initialised from counts (§ Bigram Table Prior: the clean slots
# get the direction H^T (p(next | slot) - p_b(next | curr)) at a mean norm of 10),
# with the table's peak lr 0.1 and bigram_lambdas' 0.1. Written by make_count_init.py.
...
class StackConfig:
    ...
    run_name:        str = "bi_embed_04_count_init"  # both wandb and log files
    use_wandb:       bool = True
...
# Bigram Table Prior
# ------------------------------------------------------------------------------
# A trained slot points along H^T (p(next | slot) - p_b(next | curr)): its continuation
# statistics beyond the bigram the input embeddings already carry (cos 0.35-0.38 for slots
# hit >= 10 times a step, against out-of-sample counts; agent-ops-stacks/decoder-rtx/
# 2026-09-14_0744am_bigram-hash-study, probe 2). Give the clean slots that direction at
# init. Statistics come from a replay of the first 250 steps' batches; a slot is clean
# when it has >= 256 hits there and its dominant (prev, curr) pair holds >= 50% of them
# (~32K slots, ~52% of positions); the seeded vectors are scaled to a hits-weighted mean
# norm of 10 (an input embedding row is 22).

prior_t0 = time.perf_counter()
hits = torch.zeros(cfg.d_bigram, dtype=torch.int64, device=device)
pair_keys, next_keys = [], []
replay = data_generator("train", cfg.seq_len, cfg.micro_batch_tokens, cfg.num_steps * cfg.grad_accum_steps)
for _ in range(250 * cfg.grad_accum_steps):
    inputs, targets, _ = next(replay)
    ids = inputs.to(torch.int32)
    slots = (torch.bitwise_xor(36313 * ids[1:], 27191 * ids[:-1]) % (cfg.d_bigram - 1)).to(torch.int64)
    hits += torch.bincount(slots, minlength=cfg.d_bigram)
    ids = inputs.to(torch.int64)
    pair_keys.append(ids[:-1] * cfg.d_vocab + ids[1:])
    next_keys.append(slots * cfg.d_vocab + targets[1:].to(torch.int64))
replay.close()
del replay, inputs, targets, ids, slots
hits[cfg.d_bigram - 1] = 0
pair_key, pair_count = torch.unique(torch.cat(pair_keys), return_counts=True)
next_key, next_count = torch.unique(torch.cat(next_keys), return_counts=True)
del pair_keys, next_keys

# The slot's dominant pair, and the slot each pair hashes to.
pair_prev, pair_curr = pair_key // cfg.d_vocab, pair_key % cfg.d_vocab
pair_slot = (torch.bitwise_xor(36313 * pair_curr.to(torch.int32), 27191 * pair_prev.to(torch.int32)) % (cfg.d_bigram - 1)).to(torch.int64)
dominant = torch.zeros(cfg.d_bigram, dtype=pair_count.dtype, device=device).scatter_reduce_(0, pair_slot, pair_count, reduce="amax", include_self=False)
clean = (hits >= 256) & (dominant.float() >= 0.5 * hits.float())
next_slot, next_tok = next_key // cfg.d_vocab, next_key % cfg.d_vocab

# The corpus bigram's expected head row per context, sum_j p_b(j | curr) lm_head[j], with 50 pseudo-counts of the unigram.
bigram = np.load(os.path.join(DATASET_DIR, "tokenizer/bigram_counts.npz"))
bigram_rows = torch.from_numpy(np.repeat(np.arange(cfg.d_vocab), np.diff(bigram["train_indptr"]))).to(device)
bigram_cols = torch.from_numpy(bigram["train_indices"].astype(np.int64)).to(device)
bigram_vals = torch.from_numpy(bigram["train_data"].astype(np.float32)).to(device)
del bigram
context_total = torch.zeros(cfg.d_vocab, device=device).index_add_(0, bigram_rows, bigram_vals)
next_total = torch.zeros(cfg.d_vocab, device=device).index_add_(0, bigram_cols, bigram_vals)
head_of_context = torch.zeros(cfg.d_vocab, cfg.d_model, device=device)
for s in range(0, bigram_vals.numel(), 2_000_000):
    head_of_context.index_add_(0, bigram_rows[s:s + 2_000_000], bigram_vals[s:s + 2_000_000, None] * lm_head[bigram_cols[s:s + 2_000_000]])
head_of_context = (head_of_context + 50.0 * (((next_total + 0.5) / (next_total.sum() + 0.5 * cfg.d_vocab)) @ lm_head)) / (context_total + 50.0)[:, None]
del bigram_rows, bigram_cols, bigram_vals, context_total, next_total

# g = sum_j p(j | slot) lm_head[j] - sum over the slot's pairs of p(pair | slot) head_of_context[curr].
table_prior = torch.zeros(cfg.d_bigram, cfg.d_model, device=device)
for s in range(0, next_count.numel(), 2_000_000):
    sl = next_slot[s:s + 2_000_000]
    table_prior.index_add_(0, sl, (next_count[s:s + 2_000_000].float() / hits[sl].clamp_min(1).float())[:, None] * lm_head[next_tok[s:s + 2_000_000]])
for s in range(0, pair_count.numel(), 2_000_000):
    sl = pair_slot[s:s + 2_000_000]
    table_prior.index_add_(0, sl, -(pair_count[s:s + 2_000_000].float() / hits[sl].clamp_min(1).float())[:, None] * head_of_context[pair_curr[s:s + 2_000_000]])
table_prior *= clean.unsqueeze(1)
seeded_norm = (table_prior.norm(dim=1) * hits.float()).sum() / hits[clean].float().sum()
table_prior *= 10 / seeded_norm
bigram_embeds.copy_(table_prior)
print(f"bigram table prior: {clean.sum().item():,} clean slots seeded ({hits[clean].sum().item() / hits.sum().item():.1%} of positions), "
      f"mean norm {seeded_norm.item():.1f} -> 10, {time.perf_counter() - prior_t0:.0f}s")
del hits, pair_key, pair_count, next_key, next_count, pair_prev, pair_curr, pair_slot, dominant, clean, next_slot, next_tok, head_of_context, table_prior, prior_t0, s, sl
