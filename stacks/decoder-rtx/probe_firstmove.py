...
# prior's SVD basis and least-squares target kept. Assembled by make_firstmove_probe.py.
#
...
# ==============================================================================
# § Probe: is the model's first move a consistent transformation the init could make?
# ==============================================================================
# Appended after § Weight Init & Schedule (§ Parameter Traces and later are cut; the
# prior's SVD basis and the prior's raw_target / context_weight are kept so the head can
# be re-solved for a modified embedding). Runs the real first 20 steps of v2 from the
# real init, snapshotting input_embeds and lm_head after steps 1, 2, 5, 10 and 20 and the
# grads of steps 0-2. For each move E_t - E0 (and H_t - H0) it measures how much is a
# common shift, how much is low-rank, how much is one shared linear map (fitted on even
# token ids, scored on odd ones), and how much of it is step 0's own move being undone.
# Then it restores the init, applies the measured shared moves to it (head re-solved by
# the init's own least squares), and trains 6 real steps to see what the tumult does.

V, D = cfg.d_vocab, cfg.d_model
raw_target = raw_target.cpu()                 # 4 GB; back on the device only for a head solve
...
even = (torch.arange(V, device=device) % 2) == 0

def by_bucket(values, mask=None, fmt="{:.2f}"):
    keep = torch.ones(V, dtype=torch.bool, device=device) if mask is None else mask
    cells = []
    for lo, hi, _ in buckets + [(0, 1e9, "all")]:
        sel = keep & (per_step >= lo) & (per_step < hi)
        ww = per_step[sel]
        cells.append(fmt.format((values[sel] * ww).sum().item() / ww.sum().item()))
    return " | ".join(cells)

def explained(delta, approx, mask=None):
    """Token-weighted fraction of delta's energy that approx accounts for, overall and per bucket."""
    keep = torch.ones(V, dtype=torch.bool, device=device) if mask is None else mask
    cells = []
    for lo, hi, _ in buckets + [(0, 1e9, "all")]:
        sel = keep & (per_step >= lo) & (per_step < hi)
        num = (w[sel] * (delta[sel] - approx[sel]).square().sum(1)).sum()
        den = (w[sel] * delta[sel].square().sum(1)).sum()
        cells.append(f"{(1 - num / den).item():.2f}")
    return " | ".join(cells)
...
def cosines(a, b):
    return (rows_hat(a) * rows_hat(b)).sum(1)

def mean_shift(delta):
    return ((w.unsqueeze(1) * delta).sum(0) / w.sum()).unsqueeze(0).expand(V, D)

def rank_share(delta, ks=(1, 8, 64)):
    s = torch.linalg.svdvals(sw * delta)
    tot = s.square().sum()
    return " / ".join(f"{(s[:k].square().sum() / tot).item():.3f}" for k in ks)

def shared_map(src, delta, mask):
    """A minimising sum_v w_v ||src_v A - delta_v||^2 over the rows in mask."""
    return torch.linalg.lstsq((src[mask] * sw[mask]).double(), (delta[mask] * sw[mask]).double()).solution.float()

def solve_head(E):
    """The init's own head fit: weighted least squares against the RMS-normed rows of E."""
    xe = rows_hat(E) * D ** 0.5
    tgt = raw_target.to(device)
    head = torch.linalg.solve((xe.T @ (context_weight ** 2 * xe)).double(),
                              ((context_weight ** 2 * xe).T @ tgt).double()).float().T
    del tgt
...
assert set(MUON + ADAMW) == {p.name for p in m}
...
batches = [next(train_loader) for _ in range(20 * cfg.grad_accum_steps)]
train_loader.close()
...
def train_steps(n, snap_at=(), grads_at=()):
    """n real steps from the current state. Returns the logged loss of each step, the
    (input_embeds, lm_head master) after the listed step counts, and the listed steps' grads."""
    losses, snaps, grads = [], {}, {}
    for step in range(n):
        for micro_i in range(cfg.grad_accum_steps):
            inputs, targets, cu_seqlens = batches[cfg.grad_accum_steps * step + micro_i]
            loss = forward_backward(inputs, targets, cu_seqlens, micro_step=micro_i,
                                    loss_scale=1.0 / (cfg.grad_accum_steps * inputs.size(0)))
        losses.append(loss.item())
        if step in grads_at:
            grads[step] = (m.input_embeds.grad.float().cpu(), m.lm_head.grad.float().cpu())
        for p in m:
    ...
    return losses, snaps, grads

E0 = m.input_embeds.w.float().clone()
H0 = rebuild_master(m.lm_head.w, m.lm_head.mantissa).clone()

# --- The real first 20 steps ---
losses, snaps, grads = train_steps(20, snap_at=(1, 2, 5, 10, 20), grads_at=(0, 1, 2))
print("logged loss, steps 0-19 (control: 6.049 7.609 6.563 6.415 6.239 6.120 6.025 5.939 5.924 5.869 5.810 ...):")
print("  " + " ".join(f"{l:.3f}" for l in losses), flush=True)

noise = torch.randn(V, D, device=device)
print(f"\nrank shares of a Gaussian matrix of the same shape, top-1 / 8 / 64: {rank_share(noise)}")
del noise

dE1 = snaps[1][0].to(device) - E0
dH1 = snaps[1][1].to(device) - H0
maps = {}
for t in (1, 2, 5, 10, 20):
    Et, Ht = (x.to(device) for x in snaps[t])
    dE, dH = Et - E0, Ht - H0
    print(f"\n## after {t} step(s): the move E{t} - E0 of input_embeds (rows: norm 22.2)")
    print(f"| | {bucket_header} |")
    print("|---|" + "---|" * (len(buckets) + 1))
    print(f"| L2 of the move per row | {by_bucket(dE.norm(dim=1), fmt='{:.1f}')} |")
    print(f"| angle to the init row (deg) | {by_bucket(torch.rad2deg(torch.acos(cosines(E0, Et).clamp(-1, 1))), fmt='{:.1f}')} |")
    print(f"| energy explained by a common shift (one vector for all rows) | {explained(dE, mean_shift(dE))} |")
    A_even = shared_map(E0, dE, even)
    print(f"| energy explained by E0 @ A, A a shared map fitted on even ids, scored on odd ids | {explained(dE, E0 @ A_even, mask=~even)} |")
    print(f"| (the same A scored on the even ids it was fitted on) | {explained(dE, E0 @ A_even, mask=even)} |")
    if t >= 2:
        coef = (dE * dE1).sum(1) / dE1.square().sum(1).clamp_min(1e-12)
        print(f"| coefficient of step 0's own move still present (1 = intact, 0 = undone) | {by_bucket(coef)} |")
        print(f"| energy of the move along step 0's direction | {by_bucket(cosines(dE, dE1).square())} |")
    print(f"rank share of the move (token-weighted SVD), top-1 / 8 / 64 singular directions: {rank_share(dE)}")
    maps[t] = dict(A_all=shared_map(E0, dE, torch.ones(V, dtype=torch.bool, device=device)).cpu(),
                   muE=mean_shift(dE)[0].cpu(), muH=mean_shift(dH)[0].cpu())

    print(f"\n## after {t} step(s): the move H{t} - H0 of lm_head (rows: norm {H0.norm(dim=1).mean().item():.2f})")
    print(f"| | {bucket_header} |")
    print("|---|" + "---|" * (len(buckets) + 1))
    print(f"| L2 of the move per row | {by_bucket(dH.norm(dim=1), fmt='{:.3f}')} |")
    print(f"| row norm now | {by_bucket(Ht.norm(dim=1), fmt='{:.3f}')} |")
    print(f"| angle to the init row (deg) | {by_bucket(torch.rad2deg(torch.acos(cosines(H0, Ht).clamp(-1, 1))), fmt='{:.1f}')} |")
    print(f"| energy explained by a common shift | {explained(dH, mean_shift(dH))} |")
    B_even = shared_map(H0, dH, even)
    print(f"| energy explained by H0 @ B, B a shared map fitted on even ids, scored on odd ids | {explained(dH, H0 @ B_even, mask=~even)} |")
    if t >= 2:
        coef = (dH * dH1).sum(1) / dH1.square().sum(1).clamp_min(1e-12)
        print(f"| coefficient of step 0's own move still present | {by_bucket(coef)} |")
    print(f"rank share, top-1 / 8 / 64: {rank_share(dH)}", flush=True)
    del Et, Ht, dE, dH

# --- The big gradient of step 1: how much of it is undoing step 0? ---
print("\n## step 1's gradient against step 0's move (input_embeds)")
print(f"| | {bucket_header} |")
print("|---|" + "---|" * (len(buckets) + 1))
g0, g1, g2 = (grads[t][0].to(device) for t in (0, 1, 2))
present = g1.norm(dim=1) > 0
print(f"| |g1| / |g0| per row (rows with both) | {by_bucket(g1.norm(dim=1) / g0.norm(dim=1).clamp_min(1e-30), mask=present & (g0.norm(dim=1) > 0), fmt='{:.0f}')} |")
print(f"| cos(g1, -(E1 - E0)): the gradient as the undo of step 0's move | {by_bucket(cosines(g1, -dE1), mask=present, fmt='{:.3f}')} |")
print(f"| cos(sign(g1), -(E1 - E0)): Adam's step-1 direction as the undo | {by_bucket(cosines(g1.sign(), -dE1), mask=present, fmt='{:.3f}')} |")
print(f"| cos(g1, g0) | {by_bucket(cosines(g1, g0), mask=present & (g0.norm(dim=1) > 0), fmt='{:.3f}')} |")
print(f"| cos(g2, g1) | {by_bucket(cosines(g2, g1), mask=present & (g2.norm(dim=1) > 0), fmt='{:.3f}')} |")
print(f"| energy of g1 along step 0's direction (cos^2) | {by_bucket(cosines(g1, dE1).square(), mask=present, fmt='{:.3f}')} |")
print(f"| energy of g1 explained by a shared map of E0 (fitted even, scored odd) | {explained(g1, E0 @ shared_map(E0, g1, even), mask=(~even) & present)} |")
print(f"| energy of g1 explained by a common shift | {explained(g1, mean_shift(g1), mask=present)} |", flush=True)
del g0, g1, g2

# --- Candidate inits: apply the measured shared moves up front, then train 6 real steps ---
print("\n## candidate inits: logged loss over the first 6 real steps (step 0 = the init on micro-batch 1)")
print("| init | step 0 | 1 | 2 | 3 | 4 | 5 |")
print("|---|---|---|---|---|---|---|", flush=True)
H_resolved_E0 = None
def candidate(label, E, head):
    """head: 'keep' (H0), 'resolve' (the init's least squares for E), or a tensor."""
    restore()
    if isinstance(head, str) and head == "resolve":
        H = solve_head(E)
        ...
        H = head
    set_embed_head(E, H)
    ls, _, _ = train_steps(6)
    print(f"| {label} | " + " | ".join(f"{l:.3f}" for l in ls) + " |", flush=True)

candidate("the init as is", E0, "keep")
candidate("E0, head re-solved (checks the solve reproduces the init)", E0, "resolve")
for t in (2, 10, 20):
    E_shift = E0 + maps[t]["muE"].to(device)
    candidate(f"E0 + the common shift of E{t} - E0, head kept", E_shift, "keep")
    candidate(f"E0 + the common shift of E{t} - E0, head re-solved", E_shift, "resolve")
    E_map = E0 + E0 @ maps[t]["A_all"].to(device)
    candidate(f"E0 @ (I + A{t}), A the shared map fitted on all rows, head re-solved", E_map, "resolve")
    candidate(f"E0 @ (I + A{t}) and H0 + the common shift of H{t} - H0", E_map, H0 + maps[t]["muH"].to(device))
    candidate(f"E0, H0 + the common shift of H{t} - H0", E0, H0 + maps[t]["muH"].to(device))
for t in (2, 10):
    Et, Ht = (x.to(device) for x in snaps[t])
    candidate(f"E{t} itself, head re-solved", Et, "resolve")
    candidate(f"E{t} and H{t} themselves, the rest of the model at init", Et, Ht)
    del Et, Ht
