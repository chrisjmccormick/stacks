# probe_rotation2.py
#
# Trying: whether the rotation the input_embeds rows undergo by steps 250 and 1035 of the
# control run is one map (Procrustes, cross-validated linear, per-direction rescaling)
# and whether step 0's gradient already points each row where it ends up (§ Probe at
# the end). Above it: the instrumented v2 script up to its weight init, wandb off, the
# prior's SVD basis kept. Assembled by make_rotation_probe2.py.
#
...
# ==============================================================================
# § Probe: is the rotation the rows ask for predictable at step 0?
# ==============================================================================
# Appended after § Weight Init & Schedule (§ Parameter Traces and later are cut; the
# prior's U, S and the prior tables are kept). probe_rotation.py's unconstrained 768x768
# map over-fits the few hundred frequent rows (0.2 deg residual on 44 rows is a fit, not a
# finding), so this probe uses fits that cannot do that -- an orthogonal Procrustes
# alignment, a linear map fitted on even token ids and scored on odd ones, and a per-
# direction rescaling in the prior's SVD basis -- and then asks the direct question:
# does step 0's gradient point each row toward where it ends up at steps 250 and 1035?
...
def by_bucket(values, mask=None, fmt="{:.1f}"):
    """Token-weighted mean over each frequency bucket (optionally within `mask`), then overall."""
    ...
    for lo, hi, _ in buckets + [(0, 1e9, "all")]:
        ...
        w = per_step[sel]
        cells.append(fmt.format((values[sel] * w).sum().item() / w.sum().item()))
    return " | ".join(cells)

# --- Step 0's gradient on input_embeds, as the run computes it ---
train_loader = data_generator("train", cfg.seq_len, cfg.micro_batch_tokens,
                              cfg.num_steps * cfg.grad_accum_steps)
batches = [next(train_loader) for _ in range(cfg.grad_accum_steps)]
train_loader.close()
for micro_i, (inputs, targets, cu_seqlens) in enumerate(batches):
    loss = forward_backward(inputs, targets, cu_seqlens, micro_step=micro_i,
                            loss_scale=1.0 / (cfg.grad_accum_steps * inputs.size(0)))
print(f"step 0 train loss (micro 1): {loss.item():.6f}   (control logged 6.048965)", flush=True)
g0 = m.input_embeds.grad.float().clone()          # flipped sign: the direction the update pushes
for p in m:
    p.grad.zero_()
del batches
torch.cuda.empty_cache()
present = g0.norm(dim=1) > 0                       # rows that appeared in step 0's batch

E0 = m.input_embeds.w.float().clone()
E0_hat = rows_hat(E0)
w_fit = per_step.clamp_min(1e-3)

print(f"\nrows per bucket: " + " | ".join(str(((per_step >= lo) & (per_step < hi)).sum().item()) for lo, hi, _ in buckets)
      + f" | rows with a step-0 gradient: {present.sum().item()}")

def procrustes(src, dst, w):
    """Orthogonal R minimising sum_v w_v ||src_v R - dst_v||^2."""
    Mx = src.T @ (w.unsqueeze(1) * dst)
    P, _, Qh = torch.linalg.svd(Mx.double())
    return (P @ Qh).float()

def linear_map(src, dst, w):
...
for step in (250, 1035):
    Et = torch.load(f"{ckpt_dir}/model_step{step:06d}.pt", map_location=device)["weights"]["input_embeds"].float()
    Et_hat = rows_hat(Et)
    dE = Et - E0

    print(f"\n## step {step}: token-weighted mean angle (deg) between the trained row and ...")
    print(f"| | {bucket_header} |")
    print("|---|" + "---|" * (len(buckets) + 1))
    print(f"| its init row | {by_bucket(angles(E0, Et))} |")
    R = procrustes(E0_hat, Et_hat, w_fit)
    print(f"| init row @ R, R the frequency-weighted orthogonal Procrustes rotation | {by_bucket(angles(E0_hat @ R, Et))} |")
    A = linear_map(E0_hat[even], Et_hat[even], w_fit[even])
    print(f"| init row @ A, A a linear map fitted on even token ids, scored on odd ids | {by_bucket(angles(E0_hat @ A, Et), mask=~even)} |")
    print(f"| (the same A scored on the even ids it was fitted on) | {by_bucket(angles(E0_hat @ A, Et), mask=even)} |")
    d = (w_fit.unsqueeze(1) * E0_hat * Et_hat).sum(0) / (w_fit.unsqueeze(1) * E0_hat.square()).sum(0).clamp_min(1e-12)
    print(f"| init row * d, d a per-direction rescaling in the prior's SVD basis | {by_bucket(angles(E0_hat * d, Et))} |")
    logS = S[:D].log()
    slope = torch.linalg.lstsq(torch.stack([logS, torch.ones_like(logS)], 1).double(),
                               d.clamp_min(1e-6).log().double().unsqueeze(1)).solution.squeeze()
    print(f"d by singular-value rank (mean over 96-wide bands): " + " ".join(f"{d[i:i + 96].mean().item():.3f}" for i in range(0, D, 96))
          + f"; log d vs log S slope {slope[0].item():+.3f} (0 = uniform, negative = whitening)")

    print(f"\n## step {step}: does step 0's gradient point where the row goes? (token-weighted mean cosine)")
    print(f"| | {bucket_header} |")
    print("|---|" + "---|" * (len(buckets) + 1))
    print(f"| cos(g0, E{step} - E0), rows with a step-0 gradient | {by_bucket(cosines(g0, dE), mask=present, fmt='{:.3f}')} |")
    print(f"| cos(sign(g0), E{step} - E0), i.e. Adam's step-0 direction | {by_bucket(cosines(g0.sign(), dE), mask=present, fmt='{:.3f}')} |")
    print(f"| chance level: cos(sign(g0) of another row, E{step} - E0) | {by_bucket(cosines(g0.sign().roll(1, 0), dE), mask=present, fmt='{:.3f}')} |")
    print(f"| for scale, cos(E{step} - E0, -E0): how much of the move is just leaving the init | {by_bucket(cosines(dE, -E0), fmt='{:.3f}')} |")
    print(f"| L2 of E{step} - E0 per row (the step-0 sign step is 8.3, the row 22.2) | {by_bucket(dE.norm(dim=1))} |", flush=True)
    if step == 250:
        dE250 = dE.clone()
    del Et, Et_hat

print(f"\ncos(E250 - E0, E1035 - E0): does the row keep going the way it started? | {by_bucket(cosines(dE250, dE), fmt='{:.3f}')} |")
print(f"angle(E250 - E0, E1035 - E250), the later move against the earlier one | {by_bucket(angles(dE250, dE - dE250))} |")
