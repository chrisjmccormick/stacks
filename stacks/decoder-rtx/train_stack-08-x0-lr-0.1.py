# train_stack-08-x0-lr-0.1.py -- train_stack-03-low-lr.py, plus:
# Trying: x0_lambdas' peak lr 0.5 -> 0.1 on run 3's configuration; probe 3 found their opening +-0.5 sign step (on values of 0.05-0.20) is what blew up the seeded inits, and the control's x0_lambdas swing 0.7 -> 1.65 -> 0 over the first 14 steps. Written by make_variant.py.
# (base) train_stack-03-low-lr.py -- stacks@bigram-embeds d75f72d train_stack.py, plus:
# Trying: the bigram table's peak lr 0.9 -> 0.1 and bigram_lambdas' 0.5 -> 0.1, the two
# knobs probe_table.py scored best on held-out batches; run 1's slots reached norms of
# 200-290 (an x0 row: 44) under lr 0.9. Written by make_low_lr.py.
...
class StackConfig:
    ...
    run_name:        str = "bi_embed_08_x0_lr_01"  # both wandb and log files
    use_wandb:       bool = True
