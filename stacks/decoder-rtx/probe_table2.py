# probe_table2.py
#
# Trying: an init for the hashed bigram table from counts -- whether the table trained
# for 20 steps points along directions computed from out-of-sample counts, and how
# count-derived inits at small norms score on held-out batches (§ Probe 2 at the end).
# Above it: the branch's script (stacks@bigram-embeds d75f72d: table, no gates, head
# init) up to its weight init, wandb and checkpoints off. Assembled by make_table_probe2.py.
#
...
# ==============================================================================
# § Probe 2: an init for the table from counts
# ==============================================================================
# probe_table.py found the trained table pointing along H0^T (p_hat(.|slot) - p_b(.|curr))
# (cos 0.43 by step 20), but the counts there came from the same 20 steps the table
# trained on (memorisation for rare slots), and the count-derived inits were scaled to the
# trained table's norm (~100), where the table rivals x0 in the final stream (held-out
# loss 11.0 at step 0 against 6.83). Here:
#   - the statistics come from steps 26-275 (131M positions), none of them the 20
#     training steps or the 6 held-out ones, so an alignment is generalisation;
#   - two directions: g_excess = H0^T (p_hat - p_b), the gradient-shaped one, and v_ls,
#     the least-squares fit through the head, H0 v ~ log p_hat(.|slot) - log p_b(.|curr)
#     (the direct path's target, the way the bigram prior was fitted);
#   - inits restricted to slots whose statistics are clean (>= 256 hits in the 250
#     steps, dominant pair >= 50% of them), scaled to a hits-weighted mean norm of
#     5 / 10 / 25 (an x0 row is 22), bigram_lambdas' lr 0.1 throughout (probe 1's best
#     simple knob), scored on steps 20-25 from the real init with fresh optimizer state.
...
# ---- statistics from steps 26-275, out of sample for everything above ----
...
dom_share = max_pc.float() / hits.clamp_min(1).float()
slot_n, next_n = nk // V, nk % V
rate = hits.float() / 250
touched = hits > 0
num_tokens = int(hits.sum().item())
print(f"statistics from steps 26-275: {num_tokens:,} positions, {touched.sum().item():,} slots hit, "
      f"{pk.numel():,} distinct (prev, curr) pairs, {nk.numel():,} distinct (slot, next)")

buckets = [(0, 0.1, "< 0.1 / step"), (0.1, 1, "0.1-1"), (1, 10, "1-10"), (10, 100, "10-100"), (100, 1e9, "100+")]
bucket_header = " | ".join(label for _, _, label in buckets) + " | all touched"
hF = hits.float()

def by_bucket(values, weights=None, fmt="{:.3f}"):
    weights = hF if weights is None else weights
    cells = []
...
print(f"\n## the slots by hits per step over steps 26-275 (hits-weighted means)")
print(f"| | {bucket_header} |")
print("|---|" + "---|" * (len(buckets) + 1))
print("| slots | " + " | ".join(f"{(touched & (rate >= lo) & (rate < hi)).sum().item():,}" for lo, hi, _ in buckets) + f" | {touched.sum().item():,} |")
print("| share of positions | " + " | ".join(f"{hF[touched & (rate >= lo) & (rate < hi)].sum().item() / num_tokens:.1%}" for lo, hi, _ in buckets) + " | 100% |")
print(f"| distinct (prev, curr) pairs in the slot | {by_bucket(n_pairs.float(), fmt='{:.1f}')} |")
print(f"| dominant pair's share of the slot's hits | {by_bucket(dom_share)} |", flush=True)

E0 = m.input_embeds.w.float().clone()
H0 = rebuild_master(m.lm_head.w, m.lm_head.mantissa).clone()

# ---- the corpus bigram p_b(next | curr), dense, and its expected head row ----
...
P_b = torch.zeros(V, V, device=device)
P_b[brows, bcols] = bvals
P_b = (P_b + 50.0 * unigram) / (row_tot + 50.0)[:, None]                  # (V, V), rows sum to 1
del brows, bcols, bvals, indptr
Hbar_b = P_b @ H0                                                         # (V, D)

# ---- g_excess for every touched slot (as in probe 1, out-of-sample counts) ----
G_hat = torch.zeros(S, D, device=device)
for s in range(0, nc.numel(), 2_000_000):
    e = s + 2_000_000
    G_hat.index_add_(0, slot_n[s:e], (nc[s:e].float() / hits[slot_n[s:e]].clamp_min(1).float())[:, None] * H0[next_n[s:e]])
Hbar_slot = torch.zeros(S, D, device=device)
for s in range(0, pc.numel(), 2_000_000):
    e = s + 2_000_000
    Hbar_slot.index_add_(0, slot_p[s:e], (pc[s:e].float() / hits[slot_p[s:e]].clamp_min(1).float())[:, None] * Hbar_b[curr_p[s:e]])
g_excess = G_hat - Hbar_slot
del G_hat, Hbar_slot

# ---- the least-squares direction for the clean slots ----
clean = touched & (hits >= 256) & (dom_share >= 0.5)
cidx = clean.nonzero().squeeze(1)
if cidx.numel() > 40_000:
    cidx = cidx[hits[cidx].argsort(descending=True)[:40_000]]
K = cidx.numel()
rank = torch.full((S,), -1, dtype=torch.int64, device=device); rank[cidx] = torch.arange(K, device=device)
print(f"\nclean slots (>= 256 hits in 250 steps, dominant pair >= 50%): {clean.sum().item():,}; fitted: {K:,}, "
      f"covering {hF[cidx].sum().item() / num_tokens:.1%} of positions", flush=True)
sel = rank[slot_n] >= 0
P_hat = torch.zeros(K, V, device=device)
P_hat.index_put_((rank[slot_n[sel]], next_n[sel]), nc[sel].float() / hits[slot_n[sel]].float(), accumulate=True)
P_bs = torch.zeros(K, V, device=device)                                   # the slot's own bigram mix
selp = rank[slot_p] >= 0
sp, cp, wp = rank[slot_p[selp]], curr_p[selp], pc[selp].float() / hits[slot_p[selp]].float()
for s in range(0, sp.numel(), 20_000):
    e = s + 20_000
    P_bs.index_add_(0, sp[s:e], wp[s:e, None] * P_b[cp[s:e]])
del P_b
kappa = 20.0
R = ((P_hat * hF[cidx, None] + kappa * P_bs) / (hF[cidx, None] + kappa)).log() - P_bs.log()      # (K, V) log-ratio target
R.clamp_(-10.0, 10.0)
del P_hat, P_bs
A = torch.linalg.inv((H0.T * unigram) @ H0 + 1e-6 * torch.eye(D, device=device))                # (D, D)
v_ls = ((R * unigram) @ H0) @ A                                            # (K, D): argmin_v sum_j u_j (H0_j . v - r_j)^2
del R
torch.cuda.empty_cache()
print(f"|v_ls| over the fitted slots: hits-weighted mean {(v_ls.norm(dim=1) * hF[cidx]).sum().item() / hF[cidx].sum().item():.2f}, "
      f"unweighted {v_ls.norm(dim=1).mean().item():.2f}; cos(v_ls, g_excess) hits-weighted {(F.cosine_similarity(v_ls, g_excess[cidx], dim=1) * hF[cidx]).sum().item() / hF[cidx].sum().item():+.3f}", flush=True)

# The inits, built on the CPU in bf16 so the GPU is clear for training.
def scaled(direction_rows, rows, target_norm):
    """A zero table with direction_rows on `rows`, scaled so the hits-weighted mean norm over those rows is target_norm."""
    T = torch.zeros(S, D, device=device)
    n = direction_rows.norm(dim=1)
    scale = target_norm / ((n * hF[rows]).sum() / hF[rows].sum())
    T[rows] = direction_rows * scale
    return T.to(torch.bfloat16).cpu()

inits = {}
for nu in (5.0, 10.0, 25.0):
    inits[f"v_ls on the {K:,} clean slots, mean norm {nu:.0f}"] = scaled(v_ls, cidx, nu)
    inits[f"g_excess on the same slots, mean norm {nu:.0f}"] = scaled(g_excess[cidx], cidx, nu)
wide = (touched & (hits >= 16)).nonzero().squeeze(1)
inits[f"g_excess on every slot with >= 16 hits ({wide.numel():,}), mean norm 10"] = scaled(g_excess[wide], wide, 10.0)
g_excess_cpu = g_excess.cpu(); v_ls_cpu = v_ls.cpu(); cidx_cpu = cidx.cpu()
del g_excess, v_ls, Hbar_b, E0
torch.cuda.empty_cache()

# ---- the real first 20 steps (the run's own configuration), then the generalisation test ----
...
losses = train_steps(20)
print("\nlogged loss, steps 0-19 (as is): " + " ".join(f"{l:.3f}" for l in losses))
T20 = m.bigram_embeds.w.float()
g_excess = g_excess_cpu.to(device)
print(f"\n## T20 against directions from out-of-sample counts (steps 26-275), by the slot's hits per step there")
print(f"| | {bucket_header} |")
print("|---|" + "---|" * (len(buckets) + 1))
print(f"| norm of T20 | {by_bucket(T20.norm(dim=1), fmt='{:.1f}')} |")
print(f"| cos(T20, g_excess): the gradient-shaped direction | {by_bucket(F.cosine_similarity(T20, g_excess, dim=1))} |")
cos_ls = torch.zeros(S, device=device); cos_ls[cidx] = F.cosine_similarity(T20[cidx], v_ls_cpu.to(device), dim=1)
print(f"| cos(T20, v_ls): the least-squares fit (clean slots only) | {by_bucket(cos_ls, hF * clean.float())} |")
print(f"| cos(T20, g_excess) on the clean slots | {by_bucket(F.cosine_similarity(T20, g_excess, dim=1), hF * clean.float())} |", flush=True)
del T20, g_excess
torch.cuda.empty_cache()

# ---- candidates on held-out batches (steps 20-25), fresh optimizer state ----
print("\n## candidate openings on held-out batches (steps 20-25; step 0 = the init on that batch)")
...
def candidate(label, setup=lambda: None):
    restore()
    setup()
    ls = train_steps(6, start=20)
    print(f"| {label} | " + " | ".join(f"{l:.3f}" for l in ls) + " |", flush=True)

candidate("table off (bigram_lambdas 0)", lambda: m.bigram_lambdas.w.zero_())
candidate("zero table, as is (lambda lr 0.5)")
candidate("zero table, bigram_lambdas lr 0.1", lambda: m.bigram_lambdas.lr_bc_t.mul_(0.2))
for label, T in inits.items():
    candidate(f"{label}, bigram_lambdas lr 0.1", lambda T=T: (m.bigram_embeds.w.copy_(T.to(device)), m.bigram_lambdas.lr_bc_t.mul_(0.2)))
candidate(f"v_ls on the clean slots, mean norm 10, bigram_lambdas lr 0.1 and table lr 0.3",
          lambda: (m.bigram_embeds.w.copy_(inits[f"v_ls on the {K:,} clean slots, mean norm 10"].to(device)), m.bigram_lambdas.lr_bc_t.mul_(0.2), m.bigram_embeds.lr_bc_t.mul_(1 / 3)))
candidate(f"v_ls on the clean slots, mean norm 10, lambda lr 0.5 (as is)",
          lambda: m.bigram_embeds.w.copy_(inits[f"v_ls on the {K:,} clean slots, mean norm 10"].to(device)))
