...
class StackConfig:
    ...
    run_name:        str = "probe_step1"  # both wandb and log files
...
# ==============================================================================
# § Probe: which of step 0's updates raise step 1's loss?
# ==============================================================================
# Replaces § Training Loop. Runs the real step 0 once (micro-batches 0 and 1) and
# keeps the grads. Then, per variant: restore the init weights and optimizer state,
# apply step 0's update to a subset of the parameters (optionally at a scaled lr),
# and measure the loss on all four micro-batches of steps 0 and 1 with forward-only
# passes. The training loop logs the LAST micro-batch's loss, so the run's step-0
# figure is micro 1 and its step-1 figure is micro 3.
...
batches = [next(train_loader) for _ in range(2 * cfg.grad_accum_steps)]   # steps 0 and 1
train_loader.close()                                                      # frees the pinned plan
...
ALL = MUON + ADAMW
assert set(ALL) == {p.name for p in m}

# Snapshot of the init, on the host: live weights, mantissas and moments.
snap = {p.name: {attr: getattr(p, attr).cpu() for attr in ("w", "mantissa", "first_mntm", "scnd_mntm")
                 if getattr(p, attr) is not None} for p in m}

def master(p):
    return rebuild_master(p.w, p.mantissa) if p.mantissa is not None else p.w.float()

w0 = {p.name: master(p).cpu() for p in m}

def eval_losses():
    """Mean CE in nats of the current weights on each micro-batch."""
    out = []
    for inputs, targets, cu_seqlens in batches:
        nats = forward_backward(inputs, targets, cu_seqlens, micro_step=0, backward=False)
        out.append(nats.float().mean().item())
    return out

# The real step 0: forward/backward over micro-batches 0 and 1, grads kept.
for micro_i in range(cfg.grad_accum_steps):
    inputs, targets, cu_seqlens = batches[micro_i]
    loss = forward_backward(inputs, targets, cu_seqlens, micro_step=micro_i,
                            loss_scale=1.0 / (cfg.grad_accum_steps * inputs.size(0)))
print(f"step 0 train loss (micro 1): {loss.item():.6f}   (bi_embed_log_v1 logged 6.048965)", flush=True)
...
def restore():
    for p in m:
        ...
        p.grad.copy_(grads[p.name])   # muon_step_fused lerps its grad in place

def apply_step0(names, lr_scale={}):
    """Step 0's optimizer update on the parameters in `names`; lr_scale[name] scales its lr."""
    for p in m:
...
# Size of step 0's update, per parameter.
restore()
apply_step0(ALL)
print("\n| param | rms w0 | rms dw | norm(dw)/norm(w0) | elements moved |")
print("|---|---|---|---|---|")
for p in m:
    ref = w0[p.name].to(device)
    dw = master(p) - ref
    print(f"| {p.name} | {ref.square().mean().sqrt().item():.4g} | {dw.square().mean().sqrt().item():.4g} "
          f"| {dw.norm().item() / max(ref.norm().item(), 1e-30):.3g} | {100 * (dw != 0).float().mean().item():.1f}% |")
    del ref, dw
print(flush=True)
...
groups = {
    ...
    "lm_head+input_embeds": ("lm_head", "input_embeds"),
    "value_embeds":         ("value_embeds",),
    "bigram_embeds":        ("bigram_embeds",),
    "lambdas":              ("resid_lambdas", "x0_lambdas", "bigram_lambdas", "backout_lambda"),
    "smear":                ("smear_gate", "smear_lambda"),
    "muon W_O+W_out":       ("W_O", "W_out"),
    "muon QKV+W_in":        ("W_Q", "W_K", "W_V", "W_in"),
    "muon gates":           ("attn_gate", "ve_gate"),
}
variants = [("init, no update", (), {}), ("all (the real step 0)", ALL, {})]
for label, names in groups.items():
    variants.append((f"only {label}", names, {}))
    variants.append((f"all but {label}", tuple(n for n in ALL if n not in names), {}))
for name in ("lm_head", "input_embeds", "bigram_embeds"):
    for s in (0.5, 0.25, 0.1, 0.03):
        variants.append((f"all, {name} lr x{s}", ALL, {name: s}))
for s in (0.5, 0.25, 0.1, 0.03):
    variants.append((f"all, lm_head+input_embeds lr x{s}", ALL, {"lm_head": s, "input_embeds": s}))

rows = []
print("| variant | micro 0 | micro 1 (= step 0 log) | micro 2 | micro 3 (= step 1 log) |")
print("|---|---|---|---|---|", flush=True)
for label, names, lr_scale in variants:
    restore()
    apply_step0(names, lr_scale)
    losses = eval_losses()
...
with open(f"logs/{cfg.run_name}_variants.csv", "w", newline="") as f:
    ...
    wr.writerows(rows)
print(f"rows -> logs/{cfg.run_name}_variants.csv")
