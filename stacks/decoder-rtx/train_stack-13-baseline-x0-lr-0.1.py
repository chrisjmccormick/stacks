# train_stack-13-baseline-x0-lr-0.1.py -- stacks@sep-14-baseline 19275eb train_stack.py with x0_lambdas peak lr 0.5 -> 0.1 (the change run 8 made on the branch), without the table; dev wandb project. See NOTES.md.
#
...
# Box-side fallback (2026-09-14 19:56 UTC): kernels-community/flash-attn2 vanished from the
# hub (401 unauthenticated, 404 with a token), so utils.py's get_kernel() would fail at
# import; load the build cached on this box instead. Not part of the baseline.
import kernels as _kernels
from pathlib import Path as _Path
_kernels.get_kernel = lambda *a, **k: _kernels.get_local_kernel(
    next(_Path.home().glob(".cache/huggingface/hub/models--kernels-community--flash-attn2/snapshots/*")), "flash_attn2")
...
class StackConfig:
    ...
    # Logging
    wandb_project:   str = "decoderstack_rtx_dev"  # session work
    run_name:        str = "sep14_x0_lr_0.1"  # both wandb and log files
    use_wandb:       bool = True
