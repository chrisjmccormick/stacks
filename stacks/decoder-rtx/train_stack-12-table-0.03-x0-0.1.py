# train_stack-12-table-0.03-x0-0.1.py -- train_stack-05-table-lr-0.03.py, plus:
# Trying: x0_lambdas peak lr 0.5 -> 0.1 on top of table lr 0.03 (run 5) -- run 8 found the x0 change worth ~0.01 bpb at step 125 on top of table lr 0.1. Written by make_variant.py.
# (base) train_stack-05-table-lr-0.03.py -- stacks@bigram-embeds d75f72d train_stack.py, plus:
# Trying: the bigram table's peak lr 0.9 -> 0.03 and bigram_lambdas' 0.5 -> 0.1; probe_table.py
# scored both lower lrs best on held-out batches, and run 1's slots reached norms of
...
class StackConfig:
    ...
    run_name:        str = "bi_embed_12_table_003_x0_01"  # both wandb and log files
    use_wandb:       bool = True
