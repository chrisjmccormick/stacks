# probe_rotation.py
#
# Trying: how far the input_embeds rows rotate away from the bigram-prior init by steps
# 250-1035 of the control run, and whether one linear map explains it (§ Probe at the
# end). Above it: the instrumented v2 script up to its weight init, wandb off, with the
# prior's SVD basis kept. Assembled by make_rotation_probe.py.
#
...
# ==============================================================================
# § Probe: how far do the embedding rows rotate from the bigram prior, and is it one map?
# ==============================================================================
# Appended after § Weight Init (the `del embed_prior, head_prior` line and everything
# after it are cut; `del U, S` in § Bigram Prior is removed so the SVD basis survives).
# Compares the init to the control run's checkpoints at 250/500/750/1035:
#   - per-row angle between each token's init row and its trained row, by frequency;
#   - the best single linear map A (768 x 768) from the init rows to the trained rows,
#     fitted by weighted least squares, and the per-row angle left after applying it;
#   - A expressed in the prior's SVD basis, to see whether it is a rescaling of the
#     singular directions (something the init could supply from corpus statistics);
#   - the same for lm_head, and whether the head moved by the matching inverse map.

ckpt_dir = "logs/log_v2_no_gates_no_hash"
...
tok_counts = torch.from_numpy(npz[key].astype(np.float64)).to(device).float()
per_step = tok_counts / tok_counts.sum() * cfg.total_batch_size          # occurrences per 512K-token step
buckets = [(0, 0.1, "< 0.1 / step"), (0.1, 1, "0.1-1"), (1, 10, "1-10"), (10, 100, "10-100"),
           (100, 1000, "100-1000"), (1000, 1e9, "1000+")]
...
def angles(a, b):
    return torch.rad2deg(torch.acos((rows_hat(a) * rows_hat(b)).sum(1).clamp(-1, 1)))

def by_bucket(values, weights=per_step):
    """Token-weighted mean of a per-row quantity in each frequency bucket, plus the overall."""
    cells = []
    for lo, hi, _ in buckets:
        sel = (per_step >= lo) & (per_step < hi)
        w = weights[sel]
        cells.append(f"{(values[sel] * w).sum().item() / w.sum().item():.1f}")
    cells.append(f"{(values * weights).sum().item() / weights.sum().item():.1f}")
    return " | ".join(cells)

def fit_map(src, dst, w):
    """A minimising sum_v w_v ||src_v A - dst_v||^2, via the weighted normal equations."""
    sw = w.sqrt().unsqueeze(1)
    return torch.linalg.lstsq((src * sw).double(), (dst * sw).double()).solution.float()

E0 = m.input_embeds.w.float().clone()
H0 = rebuild_master(m.lm_head.w, m.lm_head.mantissa).clone()
E0_hat, H0_hat = rows_hat(E0), rows_hat(H0)
wfit = per_step.clamp_min(1e-3)       # fit weight: token frequency (rare rows barely count)

bucket_header = " | ".join(label for _, _, label in buckets) + " | all"
print(f"\nrows per bucket: " + " | ".join(str(((per_step >= lo) & (per_step < hi)).sum().item()) for lo, hi, _ in buckets))
print(f"share of tokens: " + " | ".join(f"{100 * per_step[(per_step >= lo) & (per_step < hi)].sum().item() / per_step.sum().item():.1f}%" for lo, hi, _ in buckets))

results = {}
for step in (250, 500, 750, 1035):
    ck = torch.load(f"{ckpt_dir}/model_step{step:06d}.pt", map_location=device)
    Et = ck["weights"]["input_embeds"].float()
    Ht = ck["weights"]["lm_head"].float()          # bf16 live is enough for directions
    del ck

    ang_E = angles(E0, Et)
    ang_H = angles(H0, Ht)
    A = fit_map(E0_hat, rows_hat(Et), wfit)
    res_E = angles(E0_hat @ A, Et)
    B = fit_map(H0_hat, rows_hat(Ht), wfit)
    res_H = angles(H0_hat @ B, Ht)
    # If the embedding change were a pure gauge change, the head would move by A^-T.
    gauge_H = angles(H0_hat @ torch.linalg.inv(A).T, Ht)
    norm_E = Et.norm(dim=1)
    results[step] = dict(A=A.cpu(), B=B.cpu())

    print(f"\n## step {step}: token-weighted mean angle (deg) by occurrences per step")
    print(f"| | {bucket_header} |")
    print("|---|" + "---|" * (len(buckets) + 1))
    print(f"| input_embeds: row vs its init row | {by_bucket(ang_E)} |")
    print(f"| input_embeds: row vs (init row @ A), A the best global map | {by_bucket(res_E)} |")
    print(f"| lm_head: row vs its init row | {by_bucket(ang_H)} |")
    print(f"| lm_head: row vs (init row @ B), B the best global map | {by_bucket(res_H)} |")
    print(f"| lm_head: row vs (init row @ inv(A)^T), the gauge partner of A | {by_bucket(gauge_H)} |")
    print(f"| input_embeds row norm (init 22.2) | {by_bucket(norm_E)} |")
    sv_A = torch.linalg.svdvals(A)
    sv_B = torch.linalg.svdvals(B)
    print(f"A: singular values max {sv_A.max():.3f} median {sv_A.median():.3f} min {sv_A.min():.3f} | "
          f"||A - I||_F / ||I||_F {((A - torch.eye(D, device=device)).norm() / D ** 0.5).item():.3f} | "
          f"||A^T A - I||_F / ||I||_F {((A.T @ A - torch.eye(D, device=device)).norm() / D ** 0.5).item():.3f}")
    print(f"B: singular values max {sv_B.max():.3f} median {sv_B.median():.3f} min {sv_B.min():.3f} | "
          f"||B - I||_F / ||I||_F {((B - torch.eye(D, device=device)).norm() / D ** 0.5).item():.3f}", flush=True)

    # A in the prior's SVD basis. The init rows are (U S^0.5)_v rescaled per row, so the
    # coordinate axes already are the singular directions, ordered by S. Is A close to
    # diagonal there, and how do its diagonal entries run against S?
    diag = A.diag()
    off = (A - torch.diag(diag)).norm()
    print(f"A in the SVD basis: |diag|_F {diag.norm():.3f} vs |off-diagonal|_F {off:.3f}; "
          f"diag by singular-value rank (mean over 96-wide bands): "
          + " ".join(f"{diag[i:i + 96].mean().item():.3f}" for i in range(0, D, 96)))
    logS = S[:D].log()
    slope = torch.linalg.lstsq(torch.stack([logS, torch.ones_like(logS)], 1).double(),
                               diag.clamp_min(1e-6).log().double().unsqueeze(1)).solution.squeeze()
    print(f"  log diag(A) vs log S: slope {slope[0].item():+.3f} (0 = uniform scale, negative = whitening), "
          f"S max/min {S[0].item():.3g} / {S[D - 1].item():.3g}", flush=True)
    del Et, Ht

# Do the trained rows keep moving after 250, and along the same map?
ck250 = torch.load(f"{ckpt_dir}/model_step000250.pt", map_location=device)["weights"]["input_embeds"].float()
for step in (500, 1035):
    Et = torch.load(f"{ckpt_dir}/model_step{step:06d}.pt", map_location=device)["weights"]["input_embeds"].float()
    print(f"\ninput_embeds rows: step 250 -> step {step}, token-weighted mean angle: {by_bucket(angles(ck250, Et))}")
    del Et
torch.save(results, f"logs/probe_rotation_maps.pt")
print("maps -> logs/probe_rotation_maps.pt", flush=True)
