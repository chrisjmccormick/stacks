...
Identical to qwen-gsm8k/data/prepare_model.py except for OUT_DIR — the model
and its banked layout are the same; only the cache namespace differs, so each
speedrun folder stays self-contained.

The trainer's weights are not an nn.Module and not the checkpoint's tensors:
per-layer matrices are STACKED into (L, ...) banks, and QKV / gate-up are
...
step is launch-bound rather than bandwidth-bound. The reshape is a property of
the checkpoint, not of the run, so it happens here — the trainer opens one file
and gets tensors whose names, shapes and dtype are already the ones it uses.
...
train_qwen_arithmetic.py applies to ArithConfig.
...
# into the sidecar, where the trainer asserts them against ArithConfig — so the
